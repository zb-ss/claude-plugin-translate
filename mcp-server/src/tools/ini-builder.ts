 ...
 * INI Builder Tool
 *
 * Build, validate, and compare Joomla INI language files.
 * Supports creating new files, adding strings, validating format, and comparing
 * source/target for translation completeness.
 */
...
export const iniBuilderSchema = z.object({
  action: z.enum(["create", "add", "validate", "diff"]).describe("Action: create (new file), add (append strings), validate (check format), diff (compare files)"),
  filePath: z.string().optional().describe("Path to INI file (required for add, validate)"),
  strings: z.string().optional().describe("JSON array of strings to add: [{key, value, section?, comment?}]"),
  sourceFile: z.string().optional().describe("Source language file for diff comparison"),
  targetFile: z.string().optional().describe("Target language file for diff comparison"),
  sort: z.boolean().default(true).describe("Sort keys alphabetically within sections")
})

export type IniBuilderArgs = z.infer<typeof iniBuilderSchema>

export async function execute(args: IniBuilderArgs): Promise<string> {
